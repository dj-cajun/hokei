  ...
  return (
    <section className="border-b border-border-light bg-surface px-3 py-3">
      <div className="mb-2 flex items-center justify-between">
        ...
        <Link href={moreHref} className="text-[11px] text-muted-foreground hover:text-primary">
          더보기
        </Link>
      </div>
      <ol className="space-y-2">
        {ranked.map((item, index) => (
          <li key={item.id}>
            <Link
              href={`/posts/${item.id}`}
              className="flex items-start gap-2 rounded-lg px-1 py-1 hover:bg-card-hover"
            >
