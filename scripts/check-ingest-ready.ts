/**
 * 프로덕션 뉴스 수집 런치 점검
 * bash scripts/with-pg-env.sh npx tsx scripts/check-ingest-ready.ts
...
async function main() {
  ...
  if (neon && isPg) {
    const { getGeneratedPrismaActiveProvider } = await import(
      "../src/lib/prisma-generated-provider"
    );
    if (getGeneratedPrismaActiveProvider() !== "postgresql") {
      const { spawnSync } = await import("child_process");
      const r = spawnSync("npx", ["tsx", "scripts/prisma-generate-for-deploy.ts"], {
        stdio: "inherit",
        env: {
          ...process.env,
          PRISMA_USE_SHELL_DATABASE_URL: "1",
  ...
  if (neon && isPg) {
    ...
    prisma = createPostgresPrisma(url);
  } else {
